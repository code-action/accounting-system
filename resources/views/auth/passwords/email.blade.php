      ...
      <form class="form" method="POST" action="{{ route('password.email') }}">
        ...
        <div class="card card-login card-hidden mb-3">
          <div class="card-header card-header-info text-center">
            <h4 class="card-title"><strong>{{ __('Recuperación de Contraseña') }}</strong></h4>
          </div>
          <div class="card-body">
            @if (session('status'))
              <div class="row">
          ...
          <div class="card-footer justify-content-center">
            <button type="submit" class="btn btn-info btn-link btn-lg">{{ __('Enviar enlace para cambiar contraseña') }}</button>
          </div>
        </div>
      </form>
    </div>
